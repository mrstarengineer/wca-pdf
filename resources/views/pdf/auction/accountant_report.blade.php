...
<div class="" style="text-align: center; margin-top: -8px">
  <p style="font-size: 16px; text-transform: uppercase">For Accountant Report</p>
</div>
  ...
  <table>
    ...
    @foreach( $auction->auction_vehicles as $auctionVehicle )
      <tr>
        ...
        <td>{{ strtoupper(data_get($auctionVehicle, 'vehicle.plan')) }}</td>
        ...
        <td style="max-width: 90px; font-size: 9px">{{ data_get($auctionVehicle, 'vehicle.seller.type') == \App\Enums\MemberType::BUSINESS  ? data_get($auctionVehicle, 'vehicle.seller.company_name') :  data_get($auctionVehicle, 'vehicle.seller.name') }}</td>
      </tr>
    @endforeach
    </tbody>
  </table>
