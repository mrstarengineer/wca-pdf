...
<div class="" style="text-align: center; margin-top: -8px">
    <p style="font-size: 16px; text-transform: uppercase">Today's car listing</p>
</div>
    ...
    <table>
        ...
        @foreach( $auction->auction_vehicles as $auctionVehicle )
            <tr>
                ...
                <td>{{ \Illuminate\Support\Str::substr(data_get($auctionVehicle, 'vehicle.vin'), -4) }}</td>
                <td>{{ data_get($auctionVehicle, 'vehicle.vehicle_color.name') }}</td>
                <td>{{ strtoupper(data_get($auctionVehicle, 'vehicle.highlight.name'))}}</td>
