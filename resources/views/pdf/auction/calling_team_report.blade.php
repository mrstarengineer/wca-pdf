...
<div class="" style="text-align: center; margin-top: -8px">
    <p style="font-size: 16px; text-transform: uppercase">FOR CALLING TEAM - ON APPROVAL / ON RESERVE VEHICLES</p>
</div>
<div class="table-container">
    <table>
        ...
        @foreach( $auction->auction_vehicles as $auctionVehicle )
            @if(data_get($auctionVehicle, 'vehicle.sale_type') != \App\Enums\VehicleSaleType::PURE_SALE )
                <tr>
                    <td>{{ sprintf('%03d', $auctionVehicle->serial) }}</td>
                    <td>{{ strtoupper(data_get($auctionVehicle, 'vehicle.title')) }}</td>
                    <td>{{ strtoupper(data_get($auctionVehicle, 'vehicle.vin')) }}</td>
                    ...
                    <td>@if(data_get($auctionVehicle, 'vehicle.sale_type') == \App\Enums\VehicleSaleType::ON_RESERVE ) {{ data_get($auctionVehicle, 'vehicle.reserve_amount') }} @endif</td>
                    <td></td>
                    <td>{{ data_get($auctionVehicle, 'vehicle.seller.type') == \App\Enums\MemberType::BUSINESS  ? data_get($auctionVehicle, 'vehicle.seller.company_name') : data_get($auctionVehicle, 'vehicle.seller.name') }}</td>
                    <td>{{ data_get($auctionVehicle, 'vehicle.seller.phone_number') }}</td>
                    <td></td>
                </tr>
            @endif
        @endforeach
